...
```{r behavSplit by choice}
#Make group acc
behavData %>%
  mutate(Search.ACC = Search.ACC*100, ChoseEasy = as.factor(ChoseEasy)) %>%
  group_by(collapseNFLR, ChoseEasy) %>%
  summarise(Accuracy = mean(Search.ACC)) %>%
  ggplot(., aes(collapseNFLR, Accuracy, colour = ChoseEasy, group = ChoseEasy)) +
  geom_line(size = 2)+
  scale_x_discrete(name ="Number of easy-coloured objects") +
...
accData <- behavData %>%
  mutate(Subject = "Average") %>%
  mutate(Search.ACC = Search.ACC*100, ChoseEasy = as.factor(ChoseEasy)) %>%
  group_by(collapseNFLR, ChoseEasy, Subject) %>%
  summarise(Accuracy = mean(Search.ACC))
behavData %>%
  mutate(Search.ACC = Search.ACC*100, ChoseEasy = as.factor(ChoseEasy)) %>%
  group_by(collapseNFLR, ChoseEasy, Subject) %>%
...
behavData %>%
  ...
  ggplot(., aes(collapseNFLR, RT, colour = ChoseEasy, group = ChoseEasy)) +
  geom_line(size = 2)+
  scale_x_discrete(name ="Number of easy-coloured objects") +
...
allData %>%
    ...
    geom_point(data = N2pc.data, size = 5) +
    scale_colour_brewer(palette = "Dark2") +
>>>>>>> 38623ffc72049887cb9991d22df372e508905068
    scale_x_continuous(name ="Latency (ms)", expand = c(0, 0)) +
    scale_y_reverse(name =expression(paste("Amplitude (",mu,"v)")), expand = c(0, 0)) +
    geom_vline(xintercept = 0,linetype = "dashed" ) +
    geom_hline(yintercept = 0,linetype = "dashed") +
<<<<<<< HEAD
    theme_minimal() +
    theme(panel.spacing.y = unit(2, "lines"), text= element_text(size=60))
ggsave("SetSizeERPsSubtracted.pdf",width = plotWidth, height = plotHeight)
=======
    facet_grid(Hemisphere~SimpSet) +
    theme_minimal() +
    theme(panel.spacing.y = unit(2, "lines"), text= element_text(size=45))
