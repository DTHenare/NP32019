...
allData <- allData %>% mutate(Electrode = ifelse(EasyField==Hem, "Ipsilateral", "Contralateral"))
...
print(
  xtable(allData %>%
    filter(Event == "Search",sample == 1, Electrode == 'Contralateral') %>%
...
N2pc.data <- allData %>%
  ...
  group_by(SimpSet,Electrode,Subject) %>%
  summarise(mV = mean(voltage))
N2pc.aov <- aov_ez(
  data = N2pc.data,
  dv = "mV",
  id = "Subject", 
  within = c("Electrode","SimpSet")
)
...
#Extract for SPSS
ExportN2pc <- N2pc.data %>% unite(Condition,c("SimpSet","Electrode")) %>% spread(Condition,mV)
write.csv(ExportN2pc,"EEG\\N2pcStats.csv", row.names = FALSE)
```

The results of the interaction in the 2x3 (electrode by simplified set size) ANOVA for N2pc amplitude (`r N2pc.results$full_result$Electrode_SimpSet`).
...
The results of the interaction in the 2x3 (electrode by simplified set size) ANOVA for SPCN amplitude (`r SPCN.results$full_result$Electrode_SimpSet`).
...
allData %>%
  filter(Event == "Search") %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(EasySet,sample,Electrode) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Electrode, mean) %>% 
...
allData %>%
  filter(Event == "Search") %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(SimpSet,sample,Electrode) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Electrode, mean) %>% 
  mutate(diff = Contralateral - Ipsilateral) %>%
  ggplot(., aes(sample,diff)) +
    geom_rect(aes(xmin = N2pcStart, xmax = N2pcEnd, ymin = 0, ymax = -1), fill = "orange1") +
...
```{r indvsimpSetPlot, fig.cap="(ref:simpSetERPCap)"}
#Uses implified set sizes (1/2 vs 3/4/5 vs 6/7)
contra.small <- allData %>%
  filter(Event == "Search", Electrode=="Contralateral",SimpSet =="Small") %>%
  mutate(sample = sample*2-baseline) %>%
  group_by(Subject,SimpSet,sample) %>%
  summarise(mean = mean(voltage)) %>%
  spread(Subject, mean)
write.csv(contra.small,"EEG\\ContraData.csv", row.names = FALSE)
```
