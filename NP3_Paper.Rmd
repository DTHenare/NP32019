...
Linear modelling showed a significant effect of set size on target choice where the likelihood of responding to the easy target decreased as the number of easy-coloured rings increased (and therefore the number of difficult-coloured rings decreased) (`r apa_print(result)$full_result$collapseNFLR_L`). 

### Response times

(ref:indvSubjRT) Participant response time as a function of easy target set size (the number of difficult-coloured rings is always 8-Easy-coloured rings).

```{r plotIndvSubjRT, fig.cap="(ref:indvSubjRT)", include= FALSE}
#Make group RT
groupRT <- behavData %>%
  filter(Search.RT < Cutoff, Search.ACC==1) %>%
  mutate(Subject = "Average") %>%
...
Linear modelling showed no significant effect of set size on response times (`r apa_print(result)$full_result$collapseNFLR_L`).

```{r stzyvswitch, include=FALSE}
behavData %>%
  mutate(transition = ifelse(ChoseEasy==lag(ChoseEasy),"Stay", "Switch"), ChoseEasy = as.factor(ChoseEasy)) %>%
  filter(Search.RT < Cutoff, Search.ACC==1, Search.RT>200, !is.na(Search.RT)) %>%
  group_by(collapseNFLR, transition, ChoseEasy) %>%
  summarise(RT = mean(Search.RT)) %>%
  ggplot(., aes(x=collapseNFLR, y=RT, group=interaction(transition,ChoseEasy), color=ChoseEasy)) +
...
(ref:indvSubjAcc) Participant accuracy as a function of easy target set size (the number of difficult-coloured rings is always 8-Easy-coloured rings).

```{r plotIndvSubjAccuracy, fig.cap="(ref:indvSubjAcc)", include= FALSE}
#Make group acc
groupAcc <- behavData %>%
  filter(Search.RT < Cutoff) %>%
...
ggsave("Figures/BehaviouralPlots.tiff", dpi=300, width=32, height = 8, units = "cm")
```

Linear modelling showed no significant effect of set size on response accuracy (`r apa_print(result)$full_result$collapseNFLR_L`).
...
ggsave("Figures/N2pcTrend.tiff", plot = n2pcPlot, dpi = 300, width = 16, height = 8, units = "cm")
...
p1pcPlot <- ggplot(P1pc.data, aes(x = EasySet, y = diff, colour = Subject, group = Subject)) +
  ...
  theme(axis.line = element_line(color = "black", 
                           size = 1, linetype = "solid"))
ggsave("Figures/P1pcTrend.tiff", plot = p1pcPlot, dpi = 300, width = 16, height = 8, units = "cm")
...
SPCNPlot <- ggplot(SPCN.data, aes(x = EasySet, y = diff, colour = Subject, group = Subject)) +
  ...
  theme(axis.line = element_line(color = "black", 
                           size = 1, linetype = "solid"))
ggsave("Figures/SPCNTrend.tiff", plot = SPCNPlot, dpi = 300, width = 16, height = 8, units = "cm")
...
ggsave("Figures/ERPSPPT.tiff", dpi = 300, width = 27, height = 18, units = "cm")
```
...
ggsave("Figures/ERPSplit.tiff", dpi = 300, width = 27, height = 18, units = "cm")
...
of note:
dissociation of n2pc and spcn. in the 4:4 condition we see normal results - no p1pc, an n2pc followed by spcn. In the 1:7-3:5 conditions, we see surprise - inverse p1pc followed by n2pc followed by inverse/absent spcn. In the 5:3-7:1 conditions we also see surprise - p1pc followed by no (not much) N2pc followed by SPCN.
